...
public class them_user extends JFrame {
...
//        }
        ...
        textField_3.setBounds(131, 109, 236, 20);
        contentPane.add(textField_3);
        textField_3.setColumns(10);

        textField_4 = new JTextField();
        textField_4.setBounds(131, 140, 236, 20);
        contentPane.add(textField_4);
        textField_4.setColumns(10);

        textField_5 = new JTextField();
        textField_5.setBounds(131, 171, 236, 20);
        contentPane.add(textField_5);
        textField_5.setColumns(10);

        JLabel lblTnSch = new JLabel("Tên sinh viên");
        lblTnSch.setBounds(10, 14, 71, 14);
        contentPane.add(lblTnSch);

        JLabel lblLoiSch = new JLabel("Giới tính");
        lblLoiSch.setBounds(10, 80, 79, 14);
        contentPane.add(lblLoiSch);

        JLabel lblGiSch = new JLabel("Địa chỉ");
        lblGiSch.setBounds(10, 112, 79, 14);
        contentPane.add(lblGiSch);

        JLabel lblSLng = new JLabel("Email");
        lblSLng.setBounds(10, 143, 79, 14);
        contentPane.add(lblSLng);

        JLabel lblSNgyMn = new JLabel("Số điện thoại");
        lblSNgyMn.setBounds(10, 174, 111, 14);
        ...
        lblMSinhVin.setBounds(10, 45, 71, 14);
...
//                        + "\n" : "";
                ...
                message += !(validator.validateFieldRequired(textField_5.getText(), lblSNgyMn.getText()).equals(""))
                        ...
                        + "\n": "";
                message += !(validator.validateFieldRequired(textField_2.getText(), lblMSinhVin.getText()).equals(""))
                        ? validator.validateFieldRequired(textField_2.getText(), lblMSinhVin.getText())
                        : "";
                if (!message.equals("")) {
                    ...
                    if (!textField.getText().equals("") && !textField_3.getText().equals("") && !textField_4.getText().equals("") && !textField_2.getText().equals("") && !textField_5.getText().equals("")) {
                        if (con != null) {
                            String sql = "INSERT INTO user(us_name, us_code_student, us_gender, us_address, us_mail, us_phone_number) VALUE(?,?,?,?,?,?)";
                            try {
                                java.sql.PreparedStatement stmt = con.prepareStatement(sql);
                                stmt.setString(1, textField.getText());
                                stmt.setString(3, textField_2.getText());
                             //   stmt.setInt(2, Integer.valueOf(kho.get(comboBox.getSelectedItem())));
                                if (comboBox_1.getSelectedItem().equals("Nam")) {
                                    stmt.setInt(5, 1);
                                } else {
                                    stmt.setInt(5, 2);
                                }
                                ...
                                stmt.execute();
                                self.dispose();
                                mes.showMessage("success", "Thêm thành công!");
                            } catch (SQLException e1) {
                                // TODO Auto-generated catch block
                                e1.printStackTrace();
                            }
